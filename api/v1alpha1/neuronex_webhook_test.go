...
var _ = Describe("NeuronEX validate webhook", func() {
	...
	BeforeEach(func() {
		...
		ins = &NeuronEX{
			...
			Spec: NeuronEXSpec{
				Neuron:  corev1.Container{Name: "neuron"},
				EKuiper: corev1.Container{Name: "ekuiper"},
			},
		}
	})
	...
	It("should block if ekuiper image is empty", func() {
		Expect(k8sClient.Create(ctx, ins.DeepCopy())).Should(MatchError(matchError(ins, "ekuiper image is empty")))
	})
	It("should block create if ekuiper image is not slim or slim-python", func() {
		ins.Spec.EKuiper.Image = "lfedge/ekuiper:1.8"
		Expect(k8sClient.Create(ctx, ins.DeepCopy())).Should(MatchError(matchError(ins, "ekuiper image must be slim or slim-python")))
	...
	It("should block update if ekuiper image is slim-python or not slim-python", func() {
		...
		Expect(k8sClient.Patch(ctx, new.DeepCopy(), client.MergeFrom(ins.DeepCopy()))).Should(MatchError(matchError(ins, "ekuiper image must be slim or slim-python")))
